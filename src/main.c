...
VOID TestWriteMode(SMBIOS3_STRUCTURE_TABLE* smbiosEntry)
{
    ...
    if (!EFI_ERROR(Status))
    {
        ...
        BOOLEAN Same = CompareSmbiosTables(
            (UINT8 *)(UINTN)smbiosEntry->TableAddress,
            g_SmbiosCopy,
            g_SmbiosSize
        );
        ...
        SMBIOS_STRUCTURE_POINTER type1_copy = FindTableByType(
            smbiosEntry, 
...
EFI_STATUS efi_main(EFI_HANDLE ImageHandle, EFI_SYSTEM_TABLE *SystemTable) 
{
    ...
    switch (Key.UnicodeChar)
    {
        ...
        case L'p':
            ...
            Print(L"[INFO] Modified tables at: 0x%p\n", smbiosEntry->TableAddress);
            break;
